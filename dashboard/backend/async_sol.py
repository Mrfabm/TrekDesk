...
async def process_date(page, date, wait_time=10000):
    try:
        ...
        await page.goto("https://visitrwandabookings.rdb.rw/rdbBooking/tourismpermit_v1/TourismPermit_v1.xhtml", 
                       ...
                       timeout=wait_time)  # Use configurable wait time
...
async def scrape_slots(start_offset=0):
    """Function to be called by the scheduler"""
    db = None
    try:
        logging.info(f"Starting scrape_slots function with offset {start_offset}")
        db = SessionLocal()
        ...
        try:
            status_record = ScrapeStatus(
                status="running",
                message=f"Starting scrape for dates starting at offset {start_offset}",
            ...
            # Modify the start date based on offset
            day_after_tomorrow = datetime.now() + timedelta(days=2 + start_offset)
            results = await main(start_date=day_after_tomorrow)  # Pass start_date to main
...
async def retry_date_in_new_tab(context, date):
    try:
        ...
        # If still no result after retry, try one more time
        print(f"Still no clear result for {date}, trying one final time")
        page = await context.new_page()
        result = await process_date(page, date)
        await page.close()
        ...
        # Only if both attempts fail completely
        print(f"No clear result for {date} after all attempts")
        return None  # Return None instead of Error Loading
