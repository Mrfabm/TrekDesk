...
@router.get("/tracker")
async def get_booking_tracker_data(db: Session = Depends(get_db)):
    """
    Get comprehensive booking data for the tracker including payment status,
    authorization status, and due dates.
    """
    try:
        # Get all bookings with their related payments
        bookings = db.query(Booking).all()
        
        # Calculate payment totals and format response
        tracker_data = []
        for booking in bookings:
            ...
            tracker_data.append({
                ...
                "status": booking.status,
                "validation_status": booking.validation_status,
                "is_authorized": booking.is_authorized,
                "total_amount": booking.total_amount,
                "total_paid": total_paid,
                "payment_status": "full" if total_paid >= booking.total_amount 
