...
class FastScraper:
    def __init__(self, num_workers: int = 15):
        ...
        self.db_batch = []
        self.db_batch_size = 50  # Save to database in batches
    ...
    async def scrape(self, start_offset: int = 0):
        ...
        try:
            ...
            logger.info(f"Processing dates: {dates[0]} to {dates[-1]}")
            ...
            async with async_playwright() as p:
                ...
                logger.info(f"Completed {len(self.results)} dates in {batch_time:.2f} seconds")
...
async def scrape_slots(start_offset: int = 0):
    scraper = FastScraper()
